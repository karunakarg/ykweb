...
<table width="600" border="1" style="border:thin">
  <tr>
	...
	for($i=$num;$i!=0;$i--)
	{
		...
		{
		?>
			<a href="sitemap_data.php?q=&section=<?=$row['id']?>&category=<?=$row1['id']?>" target="_parent"><?=$row1['title']?></a><br>
		<?
        }
		echo "<hr />";
	}
	...
	for($i=$num;$i!=0;$i--)
	{
		...
		{
		?>
			<a href="../index.php?option=com_content&view=article&id=32&q=&section=<?=$row['id']?>&category=<?=$row1['id']?>" target="_parent"><?=$row1['title']?></a><br>
		<?
        }
		echo "<hr />";
	}
